...
function formatter(results, options) {
    ...
    results.forEach(function (result) {
        const code = (0, node_fs_1.readFileSync)(result.filePath, "utf-8");
    ...
    if (total > 0) {
        const problemParts = [];
        if (errors > 0) {
            problemParts.push(`${errors} ${(0, pluralize_1.default)("error", errors)}`);
        }
        if (warnings > 0) {
            problemParts.push(`${warnings} ${(0, pluralize_1.default)("warning", warnings)}`);
        }
        if (infos > 0) {
            problemParts.push(`${infos} ${(0, pluralize_1.default)("info", infos)}`);
        }
        output += (0, node_util_1.styleText)([summaryColor, "bold"], ["\u2716 ", total, (0, pluralize_1.default)(" problem", total), " (", problemParts.join(", "), ")\n"].join(""));
    }
    if (totalFixable > 0) {
        output += (0, node_util_1.styleText)([greenColor, "bold"], `✓ ${totalFixable} fixable ${(0, pluralize_1.default)("problem", totalFixable)}.\n`);
        output += `Try to run: $ ${(0, node_util_1.styleText)("underline", "textlint --fix [file]")}\n`;
    }
    if (!useColor) {
